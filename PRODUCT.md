...
- Existing writing exists to seed the blog (owner-confirmed, not yet in this repo). `content/posts/hello-world.md` is a placeholder. The owner also has a Medium account; the public feed at medium.com/@mattisig showed no published stories on 2026-09-16, so the writing may live under another handle or as drafts. Confirm the source before importing.
- Known career facts: a current role at Precisely, Göteborg (title unconfirmed); Háskóli Íslands 2013–2017 (programme unconfirmed); winner of the Code for Course challenge at the Digi.me DataHack, 2017, with the iOS app VaxAbroad. The full role history is not on hand. `content/cv.yaml` carries visible TODO placeholders; do not invent employers, dates, titles, or metrics.
